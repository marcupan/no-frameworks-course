...
export default class {
  ...
  onUpdateTitle() {
    ...
    const progress = [
      ...
      GSAP.utils.clamp(
        0,
        1,
        GSAP.utils.mapRange(0, map[2], 0, 1, -this.scroll.current - map[0] - map[1]),
      ),
      GSAP.utils.clamp(
        0,
        1,
        GSAP.utils.mapRange(0, map[3], 0, 1, -this.scroll.current - map[0] - map[1] - map[2]),
      ),
    ];
    ...
    this.titlesElement.style[this.transformPrefix] = `translateY(calc(-${y}px - 33.33% + ${
  ...
  update() {
    ...
    const index = Math.floor(
      Math.abs(
        (this.scroll.current - this.medias[0].collectionsBounds.width / 2) / this.scroll.limit,
      ) *
